...
$dbname = "z6vet51amyrj9ci0"; // Your DB name
                    ...
                    // Fetch data from the database for CBA department
                    $sql = "SELECT * FROM student_info WHERE Department = 'CBA'";
                    ...
                    <table id="violationTable">
                        <thead>
                            <tr>
                                ...
                                <th>Program</th>
                                <th>Violation</th>
                                <th>Offense</th> <!-- Added Offense column here -->
                                <th>Status</th>
                                <th>Date</th>
                                <th>Actions</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    // Determine the background color based on the offense type
                                    $offenseClass = '';
                                    if (strpos(strtolower($row['Offense']), 'major') !== false) {
                                        $offenseClass = 'offense-major';
                                    } elseif (strpos(strtolower($row['Offense']), 'minor') !== false) {
                                        $offenseClass = 'offense-minor';
                                    } elseif (strpos(strtolower($row['Offense']), 'warning') !== false) {
                                        $offenseClass = 'offense-warning';
                                    }
                                    ...
                                    echo "<tr>
                                        ...
                                        <td class='{$offenseClass}'>" . htmlspecialchars($row['Offense']) . "</td> <!-- Display Offense data here -->
    ...
    <div id="modalContent">
        ...
        <form id="addForm" action="add_cba.php" method="post">
